public class TrainLine {
    ...
    /**
     * @param String name
     * Traverses through the TrainLine to find a Station with the matching String name as given.
     * @return boolean value
     */
    public boolean contains(String name) {
        boolean contain  = false;
        //catch statement, if this.head is null everything is null
        if(this.head != null){
            TrainStation traverse = this.head;
            ...
            //check tail equivalence
            if(this.tail.getName().equals(name)){
                contain = true;
            }
        }
        return contain;
...
/**
 * @param name
 * Traverses through a TrainLine to find the TrainStation with the matching name
 * and returns the index of the equivalent station
 * @return int index
 */
    public int indexOf(String name) {
        int i = 0; //i follows the index number for iteration
        int index = -1; //If nothing matches return -1, this is int value to store index of station
        //catch statement, if this.head is null everything is null
        if(this.head != null){
            TrainStation traverse = this.head;
            while(traverse.hasNext()){
            ...
            //check tail equivalence
            if(this.tail.getName().equals(name)){
                index = this.numberOfStations - 1; 
            }
        }
        return index;
    }
/**
 * Takes all the names of stations and returns them in reverese order, last is now first.
 ...
 */
    public String reverseList() {
        ...
        if(this.head != null){ 
            TrainStation traverse = this.head;
            while(traverse.hasNext()){
                reversed = traverse.getName() + "\n" + reversed;
                traverse = traverse.getNext();
            }
            //add tail name to String
            reversed = this.tail.getName() + "\n" + reversed;
        }
        return reversed;
    }
