...
public class SimpleLinkedList {
    ...
    /**
     * Inner class for Node. Node fields can be accessed directly, for
     * simplicity of code. This is an intentional violation of the Pact.
     */
    class Node {
    ...
    public Node findMiddle() {
        ...
        if (this.head != null) {
            ...
            cursor = this.head;
            //evaluate middle
            int middle = length / two;
            //initilize traverse to keep track of number through
            int traverse = 0;
            //special check if length == 2
            if (length == two) {
                middleNode = cursor;
            } else {
                //evaluate while cursor is not null
                while (cursor != null) {
                    if (traverse == middle) {
                        middleNode = cursor;
                    }
    ...
    public SimpleLinkedList invert() {
        ...
        int i = length;
        //for length (to not go out of bounds)
        for (int k = 0; k < length; k++) {
            cursor = this.head;
            //start at the end and slowly work back from the end
            for (int j = 1; j < i; j++) {
                if (cursor.next != null) {
                    cursor = cursor.next;
                }
            ...
            //decrement i
            i--;
            //add value of cursor to beginning of new LinkedList
            inverted.add(cursor.toString());
        }      
        return inverted;
    ...
    /**
     * Driver/test code. This is some of the most embarassing code I've ever
     * written but it works. Learn from it and do not write code that bad! :-)
     */
